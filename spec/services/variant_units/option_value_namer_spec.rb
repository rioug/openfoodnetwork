  ...
  RSpec.describe OptionValueNamer do
    ...
    describe "generating option value's value and unit" do
      ...
      it "generates simple values" do
        ...
        expect(option_value_namer.send(:option_value_value_unit)).to eq [100, 'g']
      end
      ...
      it "generates values when unit value is non-integer" do
        ...
        option_value_namer = OptionValueNamer.new v
        expect(option_value_namer.send(:option_value_value_unit)).to eq [123.45, 'g']
      end
      ...
      it "returns a value of 1 when unit value equals the scale" do
        ...
        option_value_namer = OptionValueNamer.new v
        expect(option_value_namer.send(:option_value_value_unit)).to eq [1, 'kg']
      end
      ...
      it "returns only values that are in the same measurement systems" do
        ...
        option_value_namer = OptionValueNamer.new v
        expect(option_value_namer.send(:option_value_value_unit)).to eq [500, 'g']
      end
      ...
      it "generates values for all weight scales" do
        [[1.0, 'g'], [28.35, 'oz'], [453.6, 'lb'], [1000.0, 'kg'],
         [1_000_000.0, 'T']].each do |scale, unit|
          ...
          option_value_namer = OptionValueNamer.new v
          expect(option_value_namer.send(:option_value_value_unit)).to eq [10, unit]
        end
      end
      ...
      it "generates values for all volume scales" do
        [[0.001, 'mL'], [1.0, 'L'], [1000.0, 'kL']].each do |scale, unit|
          ...
          option_value_namer = OptionValueNamer.new v
          expect(option_value_namer.send(:option_value_value_unit)).to eq [3, unit]
        end
      end
      ...
      it "chooses the correct scale when value is very small" do
        ...
        option_value_namer = OptionValueNamer.new v
        expect(option_value_namer.send(:option_value_value_unit)).to eq [0.1, 'mL']
      end
      ...
      it "generates values for item units" do
        %w(packet box).each do |unit|
          ...
          option_value_namer = OptionValueNamer.new v
          expect(option_value_namer.send(:option_value_value_unit)).to eq [100, unit.pluralize]
        end
      end
      ...
      it "generates singular values for item units when value is 1" do
        ...
        option_value_namer = OptionValueNamer.new v
        expect(option_value_namer.send(:option_value_value_unit)).to eq [1, 'packet']
      end
      ...
      it "returns [nil, nil] when unit value is not set" do
        ...
        option_value_namer = OptionValueNamer.new v
        expect(option_value_namer.send(:option_value_value_unit)).to eq [nil, nil]
      end
      ...
      it "truncates value to 2 decimals maximum" do
        ...
        option_value_namer = OptionValueNamer.new v
        expect(option_value_namer.send(:option_value_value_unit)).to eq [BigDecimal(12.5, 6), 'oz']
      end
    end
  end
end
